# -*- coding: utf-8 -*-
# Taken from https://github.com/hzxie/Pix2Vox
...
class Resize(object):
    ...
    def __call__(self, rendering_images):
        ...
        if isinstance(self.size, tuple):
            ...
            h, w = rendering_images[0].shape[0:2]
            scale = self.size / min(h, w)
            dsize = (int(h * scale), int(w * scale))
        resized_images = np.empty((len(rendering_images), dsize[0], dsize[1], rendering_images.shape[-1]))
        for img_idx, img in enumerate(rendering_images):
            resized_images[img_idx] = cv2.resize(rendering_images[img_idx], (dsize[1], dsize[0]),
...
class RandomCrop(object):
    ...
    def __call__(self, rendering_images, bounding_box=None):
        ...
        sy = np.random.randint(0, rendering_images[0].shape[0] - self.crop_size_h + 1)
        sx = np.random.randint(0, rendering_images[0].shape[1] - self.crop_size_w + 1)
        processed_images = rendering_images[:, sy : sy + self.crop_size_h, sx : sx + self.crop_size_w, :]
